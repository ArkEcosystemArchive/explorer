describe('Delegate Monitor', () => {
  describe('Active Delegates', () => {
    ...
    it('should display delegate details', () => {
      ...
      cy.get('@divs').eq(0).invoke('text').should('include', 'Delegates')
      cy.get('@divs').eq(1).invoke('text').should('include', 'Total forged')
      cy.get('@divs').eq(2).invoke('text').should($text => {
    ...
    it('should be possible to click on the last block', () => {
      cy.get('h1').contains('Delegate Monitor').should('exist').then($heading => {
    ...
    it('should show forging stats for active delegates', () => {
      ...
      cy.get('@fields').eq(0).invoke('text').should('include', 'Forged block recently')
      cy.get('@fields').eq(1).invoke('text').should('include', 'Missed block')
      cy.get('@fields').eq(2).invoke('text').should('include', 'Not forging')
      cy.get('@fields').eq(3).invoke('text').should('include', 'In queue for forging')
    })

    it('should be possible to sort the active delegates', () => {
      cy.get('th', { timeout: 20000 }).eq(1).as('name').should('exist')
      cy.get('@name').invoke('text').should('include', 'Username')
    ...
    it('should be possible to click on an active delegates name', () => {
      cy.get('h1').contains('Delegate Monitor').should('exist').then($heading => {
        ...
        cy.get('tbody tr', { timeout: 20000 }).first().within(() => {
