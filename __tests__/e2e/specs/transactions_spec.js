describe('Transactions', () => {
  ...
  it('should be possible to sort the table', () => {
    cy.get('th').first().as('id').should('exist')
    cy.get('@id').invoke('text').should('include', 'ID')
  ...
  it('should be possible to click on the transaction id', () => {
    cy.get('h1').contains('Transactions').should('exist').then($heading => {
  ...
  it('should contain a dropdown allowing to filter transactions types', () => {
    cy.get('.bg-theme-feature-background').within(() => {
      ...
      cy.get('.hidden.sm\\:block .dropdown-button').contains('Vote').click()
      cy.get('span').last().invoke('text').should('include', 'Vote')
    })
  })
})
